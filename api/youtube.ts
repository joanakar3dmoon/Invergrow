  ...
  try {
    ...
    try {
      ...
      const analyticsRes = await fetch(
        `https://youtubeanalytics.googleapis.com/v2/reports?ids=channel%3D%3D${ch.id}&startDate=${fmt(startDate)}&endDate=${fmt(today)}&metrics=estimatedRevenue,views,estimatedMinutesWatched`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    await sb('invergrow_yt_stats', {
      ...
      body: JSON.stringify({
        ...
        views_30d: ytViews30d,
        watch_minutes_30d: ytWatchMinutes30d,
